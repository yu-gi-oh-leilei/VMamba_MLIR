...
def build_vssm_models_(cfg="vssm_tiny", ckpt=True, only_backbone=False, with_norm=True,
    CFGS = dict(
        ...
        vssm_base=dict(
            ...
            ckpt="/mnt/disk2/maleilei/pretrain/vssm_base_0229_ckpt_epoch_237.pth",
        ),
    ),
    ckpt_key="model",
    **kwargs):
...
def build_model(config, is_pretrain=False):
    model = None
    if model is None:
        # print("build_vssm_model: ",'===='*100)
        model = build_vssm_model(config, is_pretrain)
    if model is None:
        # print("build_heat_model: ",'===='*100)
        model = build_heat_model(config, is_pretrain)
    if model is None:
        # print("build_mmpretrain_models: ",'===='*100)
